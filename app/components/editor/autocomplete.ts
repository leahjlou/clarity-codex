...
const getToken = (line: string, replacer: string) =>
  line
    .replace(`${replacer}`, "")
    .split(" ")
    .map((t) => t.replace(new RegExp(/\)/, "g"), "")) //eslint-disable-line
    .filter((t) => t !== " ")
    .filter((t) => t !== "")
    .filter((t) => t)[0];
    ...
    provideCompletionItems: (model: editor.ITextModel, position: Position) => {
      ...
      lines.forEach((line) => {
        if (line.includes(" '")) {
          const token = line
            .split(" '")
            .map((t) => t.replace(new RegExp(/\)/, "g"), ""))[1]; // eslint-disable-line
          if (token) {
...
export function hover(monaco: Monaco) {
  monaco.languages.registerHoverProvider("clarity", {
    provideHover: (model: editor.ITextModel, position: Position) => {
      ...
      const hyphenWordPattern = new RegExp(/((?:\w+-)+\w+)/, "g"); // eslint-disable-line
